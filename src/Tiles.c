...
{
    qsort(tiles.tile, tiles.count, sizeof(*tiles.tile), CompareBySurface); // XXX. NEEDED? HOW ABOUT X INSTEAD? FIDDLE WITH MANY TREES.
...
Tiles Tiles_PrepTerrain(const Registrar terrain, const Map map, const Overview overview, const Points points)
{
    ...
    for(int32_t i = 0; i < points.count; i++)
    {
        ...
        {
            const Animation animation = terrain.animation[COLOR_NONE][file];
