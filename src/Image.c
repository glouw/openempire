...
Image Image_Load(const int32_t height, const int32_t size, FILE* const fp)
{
    ...
    // Modern compilers pad structs for word alignment.
    // The real size of the struct is inferred, and so the UTIL_FREAD macro is not used.
    ...
    // Image command table addresses are relative to SLP and
    // need to be zero adjusted to the base address.
...
SDL_Surface* Image_Parse(const Image image, const Frame frame, const Palette palette, const Color color)
{
    ...
    {
        // Scanline is transparent by default.
